                ...
                if self.flag == 7:
                    if self.tag == '302':
                        ...
                        cv2.putText(apply_color_map_image, 'Predict tag: {0}/Identification: {1}'.format(self.tag,tag_name),
                                    (probability_position_x, probability_position_y + y_move),
                                    font, fontsize, (0, 165, 255), font_scale, cv2.LINE_AA)
                        imaging = cv2.putText(imaging, pre_name+str('/Identification: {0}'.format(tag_name)), (10, 200), 2, fontsize_tag, (0, 165, 255), 2,
                                              cv2.LINE_AA)  # 文字記入
                    if self.tag == '103':
                        tag_name = 'B'
                        cv2.putText(apply_color_map_image, 'Predict tag: {0}/Identification: {1}'.format(self.tag,tag_name),
                                    (probability_position_x, probability_position_y + y_move),
                                    font, fontsize, (0, 165, 255), font_scale, cv2.LINE_AA)
                        imaging = cv2.putText(imaging, pre_name + str('/Identification: {0}'.format(tag_name)),
                                              (10, 200), 2, fontsize_tag, (0, 165, 255), 2,
                                              cv2.LINE_AA)  # 文字記入
                    if self.tag == '021':
                        tag_name = 'C'
                        cv2.putText(apply_color_map_image,
