...
class GUI:
    def __init__(self):
        ...
        self.nb = ttk.Notebook(width=1200, height=600)
        ...
        self.root.title(u"Real-time Beam Identification (predict / tag_identification / imaging) 2021 version up")
    ...
    def dnn_frame(self):
        ...
        lbl.grid(row=2, column=0,sticky=tk.W)
        lbl = tk.Label(dnnFrame, text='-----イメージング用パラメータ-----')
        lbl.grid(row=3, column=0)
        lbl = tk.Label(dnnFrame,text='初期位置設定(Initial position) 縦')
        lbl.grid(row=4, column=0,sticky=tk.W)
        lbl = tk.Label(dnnFrame,text='横')
        lbl.grid(row=4, column=2,sticky=tk.W)
        lbl = tk.Label(dnnFrame,text='画像のピクセル数(pixel) 縦')
        lbl.grid(row=5, column=0,sticky=tk.W)
        lbl = tk.Label(dnnFrame,text='横')
        lbl.grid(row=5, column=2,sticky=tk.W)
        lbl = tk.Label(dnnFrame,text='誤差修正値(0.35~0.50)')
        lbl.grid(row=6, column=0,sticky=tk.W)
        lbl = tk.Label(dnnFrame,text='-----タグ識別用パラメータ-----')
        lbl.grid(row=7, column=0,sticky=tk.W)
        ...
        initialverEntry = tk.Entry(dnnFrame,width=10)
        initialverEntry.insert(tk.END, '0')
        initialverEntry.grid(row=4,column=1,sticky=tk.W)
        initialsideEntry = tk.Entry(dnnFrame,width=10)
        initialsideEntry.insert(tk.END, '66000')
        initialsideEntry.grid(row=4, column=3, sticky=tk.W)
        pixelverEntry = tk.Entry(dnnFrame,width=10)
        pixelverEntry.insert(tk.END, '40')
        pixelverEntry.grid(row=5,column=1,sticky=tk.W)
        pixelsideEntry = tk.Entry(dnnFrame,width=10)
        pixelsideEntry.insert(tk.END, '40')
        pixelsideEntry.grid(row=5, column=3, sticky=tk.W)
        errorEntry = tk.Entry(dnnFrame,width=10)
        errorEntry.insert(tk.END, '0.44')
        errorEntry.grid(row=6, column=1, sticky=tk.W)
        ...
        def dnn_test_color_clicked():
            try:
                ...
                gainEntry_value = int(self.gainEntry.get())
                expEntry_value = int(self.expEntry.get())
                initial_height = int(initialverEntry.get())
                initial_side = int(initialsideEntry.get())
                pixel_height = int(pixelverEntry.get())
                pixel_side = int(pixelsideEntry.get())
                error_value = int(errorEntry.get())
                flip = self.flip_rdo_txt[self.flip_rdo_var.get()]
                dnn_classifier = DNNClasifier(trainEntry.get(), valEntry.get(), flip, int(epochEntry.get()), initial_height, initial_side, pixel_height, pixel_side, error_value)
        ...
        self.testcolorbutton = tk.Button(dnnFrame, text='Spectroscopic-identification-imaging', command=dnn_test_color_clicked)
        self.testcolorbutton.grid(row=6, column=4)
