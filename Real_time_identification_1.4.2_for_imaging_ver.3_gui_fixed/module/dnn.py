...
class DNNClasifier:
    def __init__(self, traindir, valdir, flip, epoch, initial_height, initial_side, pixel_height, pixel_side, error):
    ...
    def test_color(self, trigger_type, gain, exp, cvv):
        ...
        with open(os.path.join(self.sampledir, 'classname.txt')) as f:
            class_list = [s.strip() for s in f.readlines()]
        CNN(len(class_list), self.traindir, self.im_size_width, self.im_size_height,
            self.flip, self.epoch, self.initial_height,self.initial_side,self.pixel_height, self.pixel_side,self.error).cnn_test_color(trigger_type, gain, exp, class_list, cvv)